//
//  CombineLatestStateMachine.swift
//  iosApp
//
...
Base4.Element: Sendable {
    ...
    private enum State: Sendable {
        ...
        case initial(base1: Base1, base2: Base2, base3: Base3, base4: Base4?)
        
        /// The state while we are waiting for downstream demand.
        case waitingForDemand(
            task: Task<Void, Never>,
            upstreams: (Upstream<Base1.Element>, Upstream<Base2.Element>, Upstream<Base3.Element>, Upstream<Base4.Element>),
            buffer: Deque<(Base1.Element, Base2.Element, Base3.Element, Base4.Element?)>
        )
    ...
    init(
        ...
        if base4 == nil {
            ...
            self.numberOfUpstreamSequences = 4
        }
    }
    ...
    mutating func iteratorDeinitialized() -> IteratorDeinitializedAction? {
        ...
        case .waitingForDemand(let task, let upstreams, _):
            ...
            return .cancelTaskAndUpstreamContinuations(
                task: task,
                upstreamContinuations: [upstreams.0.continuation, upstreams.1.continuation, upstreams.2.continuation, upstreams.3.continuation].compactMap { $0 }
            )
    ...
    mutating func taskIsStarted(
        ...
        case .initial:
            ...
            self.state = .combining(
                task: task,
                upstreams: (.init(isFinished: false), .init(isFinished: false), .init(isFinished: false), .init(isFinished: false)),
                downstreamContinuation: downstreamContinuation,
                buffer: .init()
            )
    ...
    mutating func childTaskSuspended(baseIndex: Int, continuation: UnsafeContinuation<Void, Error>) -> ChildTaskSuspendedAction? {
        ...
        case .waitingForDemand(let task, var upstreams, let buffer):
            ...
            case 3:
                upstreams.3.continuation = continuation
    ...
    enum ElementProducedAction {
        ...
        case resumeContinuation(
            downstreamContinuation: DownstreamContinuation,
            result: Result<(Base1.Element, Base2.Element, Base3.Element, Base4.Element?)?, Error>
        )
    }
    
    mutating func elementProduced(_ result: (Base1.Element?, Base2.Element?, Base3.Element?, Base4.Element?)) -> ElementProducedAction? {
        switch self.state {
        case .initial:
        ...
        case .waitingForDemand(let task, var upstreams, var buffer):
            ...
            switch result {
            case (.some(let first), .none, .none, .none):
                buffer.append((first, upstreams.1.element!, upstreams.2.element!, upstreams.3.element))
                upstreams.0.element = first
                
            case (.none, .some(let second), .none, .none):
                buffer.append((upstreams.0.element!, second, upstreams.2.element!, upstreams.3.element))
                upstreams.1.element = second
                
            case (.none, .none, .some(let third), .none):
                buffer.append((upstreams.0.element!, upstreams.1.element!, third, upstreams.3.element))
                upstreams.2.element = third
                
            case (.none, .none, .none, .some(let fourth)):
                buffer.append((upstreams.0.element!, upstreams.1.element!, upstreams.2.element!, fourth))
        ...
        case .combining(let task, var upstreams, let downstreamContinuation, let buffer):
            ...
            if let first = upstreams.0.element,
               ...
               let fourth = upstreams.3.element {
                ...
                return .resumeContinuation(
                    downstreamContinuation: downstreamContinuation,
                    result: .success((first, second, third, fourth))
                )
                ...
                return .resumeContinuation(
                    downstreamContinuation: downstreamContinuation,
                    result: .success((first, second, third, nil))
                )
            } else {
                ...
                self.state = .combining(
                    ...
                    upstreams: (
                        ...
                        .init(continuation: upstreams.3.continuation, element: upstreams.3.element, isFinished: upstreams.3.isFinished)
                    ),
                    downstreamContinuation: downstreamContinuation,
                    buffer: buffer
                )
    ...
    mutating func upstreamFinished(baseIndex: Int) -> UpstreamFinishedAction? {
        ...
        case .waitingForDemand(let task, var upstreams, let buffer):
            ...
            if upstreams.0.isFinished && upstreams.1.isFinished && upstreams.2.isFinished && upstreams.3.isFinished {
            ...
            } else if upstreams.0.isFinished && upstreams.1.isFinished && upstreams.2.isFinished && self.numberOfUpstreamSequences == 3 {
                // All upstreams finished we can transition to either finished or upstreamsFinished now
                if buffer.isEmpty {
                    self.state = .finished
                } else {
        ...
        case .combining(let task, var upstreams, let downstreamContinuation, let buffer):
            ...
            case 3:
                upstreams.3.isFinished = true
                emptyUpstreamFinished = upstreams.3.element == nil
            ...
            } else if upstreams.0.isFinished && upstreams.1.isFinished && upstreams.2.isFinished && upstreams.3.isFinished {
                // All upstreams finished
                self.state = .finished
    ...
    enum NextAction {
        ...
        case startTask(Base1, Base2, Base3, Base4?)
    ...
    mutating func next(for continuation: DownstreamContinuation) -> NextAction {
        switch self.state {
        case .initial(let base1, let base2, let base3, let base4):
            ...
            return .startTask(base1, base2, base3, base4)
        ...
        case .waitingForDemand(let task, var upstreams, var buffer):
            ...
            if let element = buffer.popFirst() {
                ...
                let upstreamContinuations = [upstreams.0.continuation, upstreams.1.continuation, upstreams.2.continuation, upstreams.3.continuation].compactMap { $0 }
                upstreams.0.continuation = nil
                upstreams.1.continuation = nil
                upstreams.2.continuation = nil
                upstreams.3.continuation = nil
